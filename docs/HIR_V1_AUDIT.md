...
| 236 — official versioned HIR access | PASS | Zig exposes immutable checked views. C exposes additive HIR record API v2 through `vizg_hir_summary` and `vizg_hir_record_at`, while continuing to accept v1 callers; v2 supplies the previously missing instruction result `ValueId` without changing the ABI-v1 record layout. Result-owned borrowed strings remain valid for the result lifetime. `example/hir_consumer.c` builds, links, runs, and traverses all entity categories. Serialization remains absent. |
