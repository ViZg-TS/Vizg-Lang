...
pub const SymbolTypeInfo = struct {
    ...
    test "effective returns null when both are null" {
        ...
        try std.testing.expect(@isNil(entry.effective()));
    }

    test "defaults declare and infer to null" {
        const entry = SymbolTypeInfo{ .symbol_id = 0 };
        try std.testing.expect(@isNil(entry.declared_type));
        try std.testing.expect(@isNil(entry.inferred_type));
    }
};
...
pub const NodeTypeInfo = struct {
    ...
    test "nodeTypeInfo stores id and type" {
        ...
        try std.testing.expectEqual(@as(usize, 42), @intCast(n.node_id));
...
pub const TypeInfo = struct {
    ...
    test "lookupSymbol returns declared/inferred data" {
        ...
        try std.testing.expectEqual(@as(types.TypeId, 10), @intCast(found.declared_type.?));

        found = info.lookupSymbol(2) orelse unreachable;
        try std.testing.expectEqual(@as(types.TypeId, 20), @intCast(found.inferred_type.?));

        try std.testing.expect(@isNil(info.lookupSymbol(99)));
    }
    ...
    test "lookupNode returns type id" {
        ...
        try std.testing.expect(@isNil(info.lookupNode(99)));
    }
    ...
    test "empty table lookup returns null" {
        ...
        try std.testing.expect(@isNil(info.lookupSymbol(1)));
        try std.testing.expect(@isNil(info.lookupNode(1)));
    }
...
pub const Builder = struct {
    ...
    test "build returns populated table" {
        ...
        var sym = info.lookupSymbol(1) orelse unreachable;
        try std.testing.expectEqual(@as(types.TypeId, 5), @intCast(sym.declared_type.?));
